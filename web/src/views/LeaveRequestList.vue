<template>
  ...
  <div class="flex items-center justify-between flex-wrap gap-4">
      ...
      <div class="glass-panel w-full max-w-xl rounded-2xl p-6 space-y-4 max-h-[90vh] overflow-y-auto">
        ...
        <div class="bg-slate-950/30 border border-slate-850 p-4 rounded-xl text-xs space-y-3">
          ...
          <div v-if="activeRequest.letters && activeRequest.letters.length > 0" class="border-t border-slate-850 pt-2">
            <p class="font-semibold text-slate-400 mb-2">Surat Hasil Cuti (ACC):</p>
            <div class="space-y-1.5">
              <div v-for="letter in activeRequest.letters" :key="letter.id">
                <div v-if="letter.jenis_surat === 'Rekomendasi' && letter.file_signed_pdf" class="flex justify-between items-center bg-emerald-950/40 p-2 rounded border border-emerald-900/30">
                  <span class="text-emerald-400 font-bold">Rekomendasi ACC (Sudah Tanda Tangan)</span>
                  <a :href="letter.file_signed_pdf" target="_blank" class="px-2.5 py-0.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded text-[10px] font-bold shadow-md">Buka PDF</a>
                </div>
              </div>
              <div v-if="!activeRequest.letters.some(l => l.jenis_surat === 'Rekomendasi' && l.file_signed_pdf)" class="text-slate-550 italic text-[11px]">
                Surat rekomendasi belum ditandatangani / diunggah oleh Admin.
              </div>
            </div>
          </div>
        </div>
        ...
        <div v-else class="space-y-3 pt-2">
          <div class="bg-slate-900/40 border border-slate-850 p-4 rounded-xl text-xs space-y-2">
            <div class="flex justify-between">
              <span class="text-slate-500 font-semibold">Keputusan Akhir:</span>
              <span class="px-2.5 py-0.5 rounded-full text-[10px] font-semibold bg-violet-500/10 text-violet-400 border border-violet-500/20">{{ activeRequest.status }}</span>
            </div>
            <div v-if="activeRequest.catatan_admin" class="flex flex-col gap-1 pt-1.5 border-t border-slate-850">
              <span class="text-slate-500 font-semibold">Catatan Admin / Keterangan:</span>
              <span class="text-slate-300 whitespace-pre-wrap">{{ activeRequest.catatan_admin }}</span>
            </div>
          </div>
          <div class="flex justify-end border-t border-slate-800 pt-3">
            <button type="button" @click="showReviewModal = false"
                    class="px-4 py-2 bg-slate-800 hover:bg-slate-750 text-slate-300 rounded-lg text-xs font-semibold">Tutup</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <!-- DOCX PREVIEW MODAL -->
    <div v-if="showDocxPreviewModal" class="fixed inset-0 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4 z-50 overflow-y-auto">
      <div class="glass-panel w-full max-w-4xl rounded-2xl p-6 space-y-4 max-h-[95vh] overflow-y-auto flex flex-col">
        <div class="flex items-center justify-between border-b border-slate-800 pb-2">
          <h2 class="text-sm font-bold text-white">Preview Surat Rekomendasi (Word)</h2>
          <button @click="showDocxPreviewModal = false" class="text-slate-400 hover:text-white"><X class="w-5 h-5" /></button>
        </div>

        <!-- Alert for Localhost -->
        <div class="bg-amber-500/10 border border-amber-500/20 p-3 rounded-xl text-[11px] text-amber-400 space-y-1">
          <p class="font-bold">âš ï¸ Catatan untuk Pengujian Lokal:</p>
          <p>Preview dokumen Word menggunakan layanan Microsoft Office Online. Jika Anda menjalankan aplikasi ini di komputer lokal (localhost/alamat IP lokal), preview tidak akan muncul karena server Microsoft tidak dapat mengakses komputer lokal Anda secara publik. Silakan klik tombol **Unduh Dokumen Word** di bawah ini untuk mengunduhnya secara langsung.</p>
        </div>

        <!-- Download Action Button -->
        <div class="flex justify-start">
          <a :href="docxDownloadUrl" download class="px-4 py-2 bg-gradient-to-r from-primary-600 to-violet-600 hover:from-primary-700 hover:to-violet-700 text-white font-bold text-xs rounded-xl shadow-lg shadow-primary-500/20 transition-all flex items-center gap-2">
...
const openDocxPreview = (url) => {
  let absoluteUrl = url
  if (!url.startsWith('http')) {
    absoluteUrl = new URL(url, window.location.origin).href
  }
  docxDownloadUrl.value = url
  docxPreviewUrl.value = `https://view.officeapps.live.com/op/view.aspx?src=${encodeURIComponent(absoluteUrl)}`
  ...
  cols.push({
    ...
    render: (data, type, row) => {
      ...
      if (authStore.role === 'admin' && row.status === 'Diajukan') {
        buttons += `<button data-id="${row.id}" data-row="${rowJson}" class="btn-review px-2.5 py-1 bg-blue-600 hover:bg-blue-700 text-white rounded text-[10px] font-semibold transition-colors mr-1 cursor-pointer">Tinjau</button>`
      } else {
        buttons += `<button data-id="${row.id}" data-row="${rowJson}" class="btn-review px-2.5 py-1 bg-slate-750 hover:bg-slate-700 text-slate-300 rounded border border-slate-700 text-[10px] font-semibold transition-colors mr-1 cursor-pointer">Detail</button>`
      }

      // 2. Admin Actions (Upload ACC, Regenerate)
      if (authStore.role === 'admin') {
        if (['Disetujui', 'Surat Terunggah'].includes(row.status)) {
          buttons += `<button data-id="${row.id}" data-row="${rowJson}" class="btn-upload px-2.5 py-1 bg-violet-600 hover:bg-violet-700 text-white rounded text-[10px] font-semibold transition-colors mr-1 cursor-pointer">Upload ACC</button>`
        }
        if (['Disetujui', 'Surat Terunggah', 'Selesai'].includes(row.status)) {
          buttons += `<button data-id="${row.id}" class="btn-regenerate px-2.5 py-1 bg-amber-600 hover:bg-amber-500 text-white rounded text-[10px] font-semibold transition-colors mr-1 cursor-pointer">ðŸ”„ Regenerasi</button>`
        }
      }

      // 3. Document Downloads (Available to both Admin and Employee based on status)
      let dropdownItems = ''
      if (row.letters && row.letters.length > 0) {
        row.letters.forEach(letter => {
          ...
          if (letter.jenis_surat === 'Rekomendasi' && letter.file_docx) {
            dropdownItems += `<button data-url="${letter.file_docx}" class="btn-preview-docx w-full text-left px-3 py-1.5 hover:bg-slate-750 text-slate-200 text-[10px] font-medium block cursor-pointer transition-colors border-b border-slate-700/50 last:border-b-0">ðŸ“„ Rekomendasi Word</button>`
          }
          if (letter.file_pdf) {
            const icon = letter.jenis_surat === 'Formulir' ? 'ðŸ“‹' : 'ðŸ“•'
      ...
      if (dropdownItems !== '') {
        ...
        <div class="relative inline-block text-left group mr-1 align-middle">
          <button type="button" class="px-2.5 py-1 bg-indigo-700 hover:bg-indigo-650 text-slate-100 rounded text-[10px] font-semibold transition-colors inline-flex items-center cursor-pointer border border-indigo-600/50">
            Dokumen ðŸ“„ <span class="ml-1 text-[8px]">â–¼</span>
          </button>
          <div class="absolute right-0 mt-1 w-44 bg-slate-800 border border-slate-700 rounded shadow-2xl hidden group-hover:block z-[9999] overflow-hidden">
            ${dropdownItems}
          </div>
        </div>
        `
